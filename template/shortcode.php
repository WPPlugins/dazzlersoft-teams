...
add_shortcode( 'TEAM_M', 'TEAM_M_ShortCode' );
function TEAM_M_ShortCode( $Id ) {
	ob_start();	
	if(!isset($Id['id'])) 
	 {
		$dazzler_Team_ID = "";
	...
	{
		$dazzler_Team_ID = $Id['id'];
	}
	
	$post_type = "team_member";
	$AllTeams = array(  'p' => $dazzler_Team_ID, 'post_type' => $post_type, 'orderby' => 'ASC');
	...
	while ( $loop->have_posts() ) : $loop->the_post();
		...
		$All_data = unserialize(get_post_meta( $PostId, 'dazzler_team_m_data', true));
		$TotalCount =  get_post_meta( $PostId, 'dazzler_team_m_count', true );
